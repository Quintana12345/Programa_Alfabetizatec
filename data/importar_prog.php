...
try {
    ...
    foreach ($filas as $fila) {
        ...
        $nombre = $fila[0];  // Columna A (Nombre)
        $apellido_paterno = $fila[1];  // Columna B (Apellido Paterno)
        $apellido_materno = $fila[2];  // Columna C (Apellido Materno)
        $sexo = $fila[3];  // Columna D (Sexo)
        $curp = $fila[4];  // Columna E (CURP)
        $correo_inst = $fila[5];  // Columna F (Correo electrónico)
        $telefono = $fila[6];  // Columna G (No. de teléfono celular)
        $puesto = $fila[7];  // Columna H (Cargo)
        $tipo_institucion = $fila[8];  // Columna I (Tipo de Institución)

        // Verificación para determinar el nombre del tecnológico entre la columna K (índice 9) o L (índice 10)
        $nombre_tecnologico = empty($fila[9]) ? $fila[10] : $fila[9];  // Columna K o L (nombre tec / subnombre)
        ...
        $stmtUsuario->bind_param("sssssssi", $nombre, $apellido, $sexo, $correo_inst, $curp, $puesto, $contrasena, $rol_id);
